    ...
    _occChart = new Chart(ctx, {
        ...
        options: {
            ...
            scales: {
                ...
                y: {
                    beginAtZero: true, ticks: { font: { size: 10 }, color: 'rgba(100,116,139,0.9)', stepSize: 1 },
...
window.onload = function () {
    ...
    window.dbExportCSV = function (type) {
        ...
        var prefix;
        if (type === 'energy_ac' || type === 'energy_lamp' || type === 'energy_outlet' || type === 'energy_total') {
            prefix = 'energy';
        } else if (type === 'ga_fitness') {
            prefix = 'ga_fitness';
        } else if (type === 'pso_fitness') {
            prefix = 'pso_fitness';
        } else {
            prefix = type;
        }
        ...
        fetch(url).then(function (r) {
            if (r.ok && r.headers.get('Content-Type') && r.headers.get('Content-Type').indexOf('text/csv') >= 0) {
                return r.blob().then(function (blob) {
                    ...
                    showToast('CSV berhasil didownload: ' + fname, 'success');
                });
            } else {
